...
def main() -> None:
    ...
    print(f"is_seoul 매칭: {int(df['is_seoul'].sum())}건")

    df = mdis.apply_winsorize(df)
    df = mdis.build_treatment_vars(df)
    print(f"treat_binary 분포: {df['treat_binary'].value_counts().to_dict()} (assert 통과)")

    stage_a, stage_b = mdis.split_stage_ab(df)
    print(f"stage_a: {len(stage_a)}행 / stage_b: {len(stage_b)}행 (assert 통과)")
