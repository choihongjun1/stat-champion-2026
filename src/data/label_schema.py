...
VARIABLE_DEFINITIONS = {
    ...
    "origin_end": "origin 분기의 마지막날. feature_asof/age_months뿐 아니라 적격 조건(인허가일자<=origin_end<(폐업일자 또는 무한대))과 12개월 라벨 창 기준 시점이기도 하다 - origin_start를 쓰면 feature_asof 시점에 이미 폐업한 점포가 라벨에 남는 시간 누수가 생기므로 전부 origin_end로 통일했다.",
    "event_12m": "origin_end 이후 12개월 이내(origin_end < 폐업일자 <= origin_end+12개월) 폐업일자 존재 시 1, 아니면 0. feature_asof(=origin_end) 기준으로 통일해 시간 누수를 방지한다.",
    "age_months": "파생변수. origin_end 기준 (origin_end - 인허가일자) 개월수. 음수 불가(assert) - 패널 진입 조건상 인허가일자<=origin_end가 이미 보장됨.",
