...
2. 인허가 ↔ 소진공 매칭
   ...
   - ⚠️ **미합의 — 겹침 조건**: 이 문서는 원래 "인허가 영업기간과 소진공 존재구간이
     1분기 이상 겹칠 것"을 매칭 필수 조건으로 적었으나, 구현된 ER은 겹침을 검사하지
     않는다(`DECISIONS.md` 2026-09-19). **아직 어느 쪽으로도 확정하지 않았다.**
     선택지와 영향은 아래 "B-3 겹침 조건 (미합의)" 절 참조.
3. 보조 컬럼 생성
...
ER은 "같은 점포인가"만 판정하고 시점 겹침은 보지 않는다. 겹침을 어디서 쓸지 정해야 한다.

| 안 | 내용 | 영향 |
|---|---|---|
| **A. ER hard filter** | 겹침이 기준 미만이면 매칭 자체를 무효로 본다 | 현재 matched 27,430건 중 **6,105건(22.3%)이 영향**(90일 기준 실측). 동일 필지의 이전/신규 점포 혼동은 줄지만, SEMAS 관측이 2024-12~2026-06뿐이라 **최근 개업·오래전 폐업 점포가 구조적으로 탈락**한다(경계 효과). 재발급으로 이어 붙인 entity는 관측구간이 길어져 판정이 tier에 따라 달라진다 |
| **B. B-3 QA/provenance** | ER은 그대로 두고 `overlap_days`를 계산해 `sj_status` 판정과 민감도 분석에만 쓴다 | 매칭 수 불변. 겹침이 짧은 건을 `sj_status`에서 낮은 신뢰도로 표시하고, 보고서에서 민감도로 비교. complete-case 금지 원칙과 충돌하지 않음 |

- 어느 쪽이든 **주 폐업 라벨(인허가 폐업일자)은 바뀌지 않는다.**
- 기준 단위도 함께 정해야 한다 — 이 문서는 "1분기", `DECISIONS.md`·`DATA_CATALOG.md`의
  실측은 "90일" 기준이다. 두 값은 서로 다른 기준이며 아직 통일되지 않았다.
- 결정하면 `DECISIONS.md`에 기록하고 이 절을 갱신한다.
